...
const beforeUpload = (file: File) => {
  ...
  if (!isImage) {
    message.error('File harus berupa JPG, PNG, JPEG, atau WEBP')
    return false
  }
...
const handleSave = async () => {
  try {
    ...
    if (form.value.file?.url && !form.value.file?.originFileObj) {
      imageUrl = form.value.file.url
    }

    if (form.value.file?.originFileObj) {
      const response = (await uploadImage(
        form.value.file.originFileObj as File
      )) as UploadResponse
    ...
    message.success('Collection section berhasil disimpan')
  } catch (error) {
    console.error(error)
    message.error('Gagal menyimpan collection section')
  } finally {
    loading.value = false
  }
}
